...
clear 
%% converting
data=readtable("C:\Users\mordi\OneDrive\Desktop\master degree\LAB\Infra_Red\All_Data.csv");
n=size(data);
for i=1:n(1)
    if strcmp(data{i,1},'CT13')
        switch char(data.Var2(i))
            ...
            case 'sugar preference test'
                if strcmp(data{i,3},'Right')
                    newdata.CT13.SP.Right=data{i,4:9};
                else
                    newdata.CT13.SP.Left=data{i,4:9};
                end
        ...
        switch char(data.Var2(i))
            ...
            case 'sugar preference test'
                if strcmp(data{i,3},'Right')
                    newdata.T24.SP.Right=data{i,4:9};
                else
                    newdata.T24.SP.Left=data{i,4:9};
                end
            case 'forced swiming test'
                if strcmp(data{i,3},'Frequency')
